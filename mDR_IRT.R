...
rmsea_drm <- function(fit.ref, fit.comp){
  ...
  if(lambda > 0){
    ci90 <- sqrt(
      (qchisq(c(0.05, 0.95), df = df, ncp = lambda) - df)/ (df * sample_size)
    )
  } else {
    ci90 <- NULL
  }
...
cfa_helper_matrices <- function(formula_string, data){
  ...
  init_mat <- matrix(rep(c((d:1)/d,0,log(10)), each=ncol(data)), nrow=ncol(data))
  ...
  temp[is.na(temp)] <- 0
  for(i in 1:max(temp)){
    init_mat[temp == i] <- mean(init_mat[temp == i])
  }
...
efa_helper_matrices <- function(d, data){
  if(is.null(colnames(data))){
    cns <- paste0("v", 1:ncol(data))
  } else {
    cns <- colnames(data)
...
dr2_helper_matrices <- function(formula_string, data, initial_nu){
  if(is.null(colnames(data))){
    cns <- paste0("v", 1:ncol(data))
  } else {
    cns <- colnames(data)
  ...
  init_mat[,1] <- log(initial_nu)
  for(i in 1:nrow(init_mat)){
    init_mat[i, 2:(category[i]+1)] <- 0
  }
  load_mat <- matrix(1, nrow = nrow(init_mat), ncol = ncol(init_mat))
  load_mat[is.na(init_mat)] <- 0
  
  fns <- c("nu", paste0("t", 1:max(category)))
  
  par_id <- init_mat
  par_id[!is.na(par_id)] <- 1:sum(!is.na(par_id))
...
efa2_helper_matrices <- function(d, data, eq_interval = FALSE){
  if(is.null(colnames(data))){
    cns <- paste0("v", 1:ncol(data))
  } else {
    cns <- colnames(data)
...
cfa2_helper_matrices <- function(formula_string, data, eq_interval = FALSE){
  if(is.null(colnames(data))){
    cns <- paste0("v", 1:ncol(data))
  } else {
    cns <- colnames(data)
...
Mstep_sim <- function(E, item, par_id=NULL, grouping=NULL, ngrid = 1000,
  ...
  repeat{
    ...
    for(i in 1:length(grouping)){
      ...
      if(calculate_m){
        missing_I <- IMm[grouping[[i]],grouping[[i]]] - L1[grouping[[i]]] %*% t(L1[grouping[[i]]])
...
dr_sim <- function(data, dimension=NULL, range=c(-4,4), q=11, t_prior=0.25, initialitem=NULL, ngrid=1000,
  ...
  repeat{
    ...
    if(estimation == "EM"){
      ...
      flush.console()
      if((iter > 11) & (iter >= max_iter | grad_conv < threshold) & (diff < threshold2)) break
    } else if(estimation == "MHRM"){
  ...
  if (estimation =="EM"){
    ...
    return(structure(
      list(par_est=initialitem,
           se=M[c("se", "se_raw", "inv_H", "inv_H_raw")],
           fk=E$freq,
           iter=iter,
           quad=grid,
           diff=diff,
...
efa_sim <- function(data,dimension,t_prior=0.25,range=c(-4,4),q=41,ngrid=1000,max_iter=NULL,
  ...
  helper_matrices <- efa2_helper_matrices(dimension, data, eq_interval = eq_interval)
...
cfa_sim <- function(formula,data,t_prior=0.25,range=c(-4,4),q=41,ngrid=1000,max_iter=NULL,
  ...
  helper_matrices <- cfa2_helper_matrices(formula, data, eq_interval = eq_interval)
